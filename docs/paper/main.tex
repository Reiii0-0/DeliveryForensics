...
\author{
\IEEEauthorblockN{1\textsuperscript{st} Farikh Muhammad Fauzan}
\IEEEauthorblockA{\textit{DustiniaDelixia Groceria Research Group} \\
Jakarta, Indonesia \\
tech@dustinia.com}
\and
\IEEEauthorblockN{2\textsuperscript{nd} Farikh Muhammad Fauzan}
\IEEEauthorblockA{\textit{Dept. of Informatics Eng., FTEIC} \\
\textit{Institut Teknologi Sepuluh Nopember} \\
Surabaya, Indonesia \\
5025241092@student.its.ac.id}
}

\maketitle

\begin{abstract}
The rapid growth of global e-commerce has introduced extreme stochastic variability into last-mile logistics. Traditional descriptive metrics, such as average lead times and aggregated On-Time Delivery Rates (OTDR), often mask systemic disorder and structural vulnerabilities within supply chain networks. This paper proposes a practical, information-theoretic framework named \textit{DeliveryForensics}. We utilize Multi-state Shannon Entropy to quantify operational uncertainty and an additive What-If Sensitivity Analysis to evaluate recovery pathways. Built on a high-throughput ELT pipeline leveraging the ClickHouse columnar engine, our system enables real-time forensic diagnostics on large-scale transactional datasets. We evaluate our approach using the Olist Brazilian E-Commerce dataset (n=96,455), revealing a three-tier hub taxonomy that identifies Geographic Variability Hubs in high-volume states, Low-Entropy Failure Hub candidates in isolated regions, and borderline Chaos Hub candidates. Furthermore, we demonstrate via simulation that a 20\% optimization in targeted seller preparation stages yields a projected OTDR of 93.49\% [bootstrap 95\% CI: 93.42\%, 93.57\%], representing a statistically significant improvement of 1.60 percentage points toward the 95\% SLO target. Our methodology provides a rigorous, complementary paradigm for automated, high-performance logistics auditing.
\end{abstract}
...
Recent advancements have characterized supply chain complexity as a quantifiable system property rather than an external constraint \cite{wang2016bullwhip}. However, there remains a significant gap between theoretical complexity measures and real-time industrial implementation. This gap is not unique to the Brazilian market; Mahmoudi et al.\ \cite{mahmoudi2026real} identify analogous KPI monitoring challenges in non-food retail supply chains, highlighting the need for real-time analytical frameworks applicable across diverse logistics environments. To bridge this, we propose \textit{DeliveryForensics}, an advanced investigative system that transitions logistics monitoring from descriptive reporting to prescriptive diagnostics. 
...
\begin{tikzpicture}[
    ...
    \node[box, below=of fact] (dim) {Dims \& Geo\\(Haversine)};
    ...
    \draw[arrow] (fact) -- (dim);
    \draw[arrow] (dim) -- (mv);
    \draw[arrow] (staging) -- node[left] {\scriptsize stateful agg} (mv);
...
\caption{The Dual-Core DeliveryForensics Architecture. Airflow orchestrates the pipeline while ClickHouse natively executes stateful aggregations (e.g., \texttt{entropyState}).}
...
\begin{lstlisting}[caption={Materialized View for Multi-state Entropy}]
...
\end{axis}
\end{tikzpicture}
\caption{Box plot of delivery days by distance. Whiskers represent 5th and 95th percentiles, computed directly from Olist dataset using ClickHouse \texttt{quantile()}. Note: Sample sizes in the 0--250\,km bucket increased substantially in a prior revision following correction of a geolocation pipeline error.}
\label{fig:boxplot}
\end{figure}

\subsection{Identification of Chaos Hubs}
To ensure statistical reliability of entropy estimates, we restricted this analysis to distribution hubs with a minimum of 500 transactions. Table II reveals the empirical state-level distribution and classification.
...
Note: Stage-level entropy values use binary threshold-exceedance classification ($H_{\max} = 1.0$ bit), distinct from the 4-state FEI ($H_{\max} = 2.0$ bits) used for geographic analysis. These metrics are not directly comparable and should be interpreted as independent diagnostics. We analyzed the informational noise across the fulfillment lifecycle using binary threshold exceedance for each stage. Using operationally-motivated binary thresholds (Approval: $>24$h, Preparation: $>48$h, Transit: $>10$ days), Preparation Entropy ($H=0.995$ bits) was found higher than Transit Entropy ($H=0.886$ bits) and Approval Entropy ($H=0.626$ bits). However, we note that the transit threshold (10 days) is substantially more permissive than the preparation threshold (48 hours); a sensitivity analysis using a tighter transit threshold (e.g., $>3$ days) may alter this ranking. Under the thresholds chosen, seller preparation represents the most entropy-generating stage, suggesting it as a priority for variance reduction. Future work will harmonize stage thresholds for cross-stage comparability using a consistent 4-state framework.
...
Delayed orders received significantly lower review scores ($Mdn = 2.0, n = 7,699$) compared to on-time orders ($Mdn = 5.0, n = 88,639$), $U = 5.98 \times 10^7$, $p < 0.001$, rank-biserial $r = 0.82$. (Note: 117 orders of the 96,455 total were excluded due to missing review data). This large effect size confirms a strong association between delivery status and customer satisfaction. However, we note that this association does not establish causality, as confounding factors may contribute to both variables.
...
\section{Conclusion}
The \textit{DeliveryForensics} system demonstrates that the combination of multi-state entropy diagnostics and what-if sensitivity analysis provides a complementary framework to traditional logistics auditing. By utilizing ClickHouse's columnar execution for in-database analytics, we achieved sub-second latency on complex uncertainty metrics. This integrated approach enables operators to systematically classify distribution nodes into a three-tier hub taxonomy: \textit{Chaos Hubs} (high entropy co-occurring with high SLA non-compliance, requiring process variance reduction), \textit{Geographic Variability Hubs} (high entropy driven by route diversity, requiring SLA-threshold recalibration), and \textit{Low-Entropy Failure Hubs} (low entropy with high SLA non-compliance, requiring infrastructure investment). Each hub type demands a categorically distinct operational prescription --- a diagnostic granularity that average-based metrics fundamentally cannot provide. The What-If Sensitivity Analysis further quantifies the recovery potential of targeted stage-level interventions under bounded uncertainty, providing operators with empirically grounded scenario planning. Furthermore, consistent with supply chain resilience frameworks \cite{liu2022leveraging}, our findings highlight that such analytical capabilities must be complemented by organizational disruption orientation to effectively navigate the stochastic nature of modern e-commerce fulfillment.

\section*{Data and Code Availability}
The Olist Brazilian E-Commerce Public Dataset used in this study is publicly available \cite{olist2018}. Preprocessing criteria required \texttt{status="delivered"} and complete timestamp records. Code availability, including core SQL queries shown in Listings 1 and 2 and the full implementation scripts, are available at \url{https://github.com/rei/DeliveryForensics} under the MIT License.
